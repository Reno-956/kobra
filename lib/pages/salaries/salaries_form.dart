...
class SalariesForm extends StatefulWidget {
  final UsuarioModel? selectedUser;
                  ...
                  Wrap(
                    children: [
                      if (users.hasValue && users.value?.isNotEmpty == true)
                        ShadSelectFormField<UsuarioModel>(
                          id: 'user',
                          ...
                          options: users.value!
                              .map((u) =>
                                  ShadOption(value: u, child: Text(u.nombre)))
                              .toList(),
                  ...
                  Row(
                    children: [
                      ...
                      reportBonus.map(
                        data: (data) {
                          return Text(
                              'Hitungan ${currency.format(data.fold(0, (p, c) => p + c.precioTotal.toInt()))} >> Bonus : ${currency.format(data.fold(0, (p, c) => p + c.precioTotal.toInt()) * 1 / 100)}');
                        },
