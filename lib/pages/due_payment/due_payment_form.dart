...
class DuePaymentForm extends HookWidget {
  ...
  Widget build(context) {
    ...
    final editingName = useTextEditingController(text: item?.nombre ?? '');
    final editingInvoice = useTextEditingController(text: item?.factura ?? '');
    final editingItemName =
        useTextEditingController(text: item?.itemNombre ?? '');
    final editingAmount =
        useTextEditingController(text: (item?.cantidad ?? '').toString());
    final editingNote =
        useTextEditingController(text: (item?.nota ?? '').toString());

    final stock = useState(item?.itemCantidad ?? 0);
    final hargaJual = useState(0.0);
    final hargaJualDiscount = useState(0.0);
    final status = useState(item?.estado ?? 'dept');
    final dateIn = useState(item?.fechaIngreso ?? DateTime.now());
    final dueDate = useState(
        item?.fechaVencimiento ?? DateTime.now().add(const Duration(days: 3)));
                    ...
                    Expanded(
                      child: Container(
                        ...
                        child: ShadSelect<int>(
                          ...
                          initialValue: item?.itemCantidad,
                      ...
                      ShadButton(
                        ...
                        onPressed: () {
                          if (!duePaymentFormKey.currentState!.validate()) {
                            ...
                            if (item != null) {
                              final updateitem = MetodoPagoModel(
                                id: item.id,
                                nombre: editingName.text.replaceAll(',', ' '),
                                factura: editingInvoice.text,
                                itemNombre: editingItemName.text,
                                itemCantidad: stock.value,
                                cantidad: int.parse(editingAmount.text),
                                estado: status.value,
                                fechaIngreso: dateIn.value,
                                fechaVencimiento: dueDate.value,
                                nota: editingNote.text,
